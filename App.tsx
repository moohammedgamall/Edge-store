...
const App: React.FC = () => {
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-32 animate-in fade-in duration-500">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 font-medium">Manage your digital assets and security.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => { setIsChangingPassword(!isChangingPassword); setIsEditing(false); setIsEditingBanner(false); }} className={`px-6 py-3 rounded-xl font-bold shadow-lg transition-all ${isChangingPassword ? 'bg-zinc-800 text-white' : 'bg-white text-zinc-900 border border-zinc-200'}`}><i className="fa-solid fa-lock mr-2"></i> Security</button>
          <button onClick={() => { setIsEditingBanner(!isEditingBanner); setIsEditing(false); setIsChangingPassword(false); }} className={`px-6 py-3 rounded-xl font-bold shadow-lg transition-all ${isEditingBanner ? 'bg-zinc-800 text-white' : 'bg-white text-zinc-900 border border-zinc-200'}`}><i className="fa-solid fa-pen-fancy mr-2"></i> Banner</button>
          <button onClick={() => { setIsEditing(true); setIsEditingBanner(false); setIsChangingPassword(false); setEditProduct({ id: Date.now().toString(), price: 0, category: 'Themes', rating: 5.0, downloads: '0', isPremium: false, gallery: [] }); }} className="px-6 py-3 bg-[#007AFF] text-white rounded-xl font-bold shadow-lg shadow-blue-500/20"><i className="fa-solid fa-plus-circle mr-2"></i> Add Item</button>
        </div>
      </header>
      ...
      {isChangingPassword && (
        <div className="glass-panel p-10 rounded-[2rem] space-y-6 animate-in slide-in-from-top-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <input type="password" placeholder="Current Password" className="p-4 rounded-xl border bg-white" value={newPasswordData.current} onChange={e => setNewPasswordData({...newPasswordData, current: e.target.value})} />
            <input type="password" placeholder="New Password" className="p-4 rounded-xl border bg-white" value={newPasswordData.next} onChange={e => setNewPasswordData({...newPasswordData, next: e.target.value})} />
            <input type="password" placeholder="Confirm New" className="p-4 rounded-xl border bg-white" value={newPasswordData.confirm} onChange={e => setNewPasswordData({...newPasswordData, confirm: e.target.value})} />
          </div>
          <button onClick={handleUpdatePassword} className="w-full py-4 bg-zinc-900 text-white rounded-xl font-black">Update Password</button>
        </div>
      )}
      ...
      {isEditingBanner && (
        <div className="glass-panel p-10 rounded-[2rem] space-y-6 animate-in slide-in-from-top-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <input placeholder="Title" className="p-4 rounded-xl border bg-white" value={banner.title} onChange={e => setBanner({...banner, title: e.target.value})} />
            <input placeholder="Highlight" className="p-4 rounded-xl border bg-white" value={banner.highlight} onChange={e => setBanner({...banner, highlight: e.target.value})} />
            <input placeholder="Image URL" className="p-4 rounded-xl border bg-white col-span-2" value={banner.imageUrl} onChange={e => setBanner({...banner, imageUrl: e.target.value})} />
          </div>
      ...
      {isEditing && (
        <div className="glass-panel p-10 rounded-[2rem] space-y-8 animate-in zoom-in-95">
          <div className="flex justify-between items-center">
            <h3 className="text-2xl font-black tracking-tight">Product Configuration</h3>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="space-y-1.5">
                  ...
                  <input placeholder="Product Title" className="w-full p-4 rounded-xl border bg-white font-bold" value={editProduct.title || ''} onChange={e => setEditProduct({...editProduct, title: e.target.value})} />
               </div>
               <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-2">Price ($)</label>
                  <input placeholder="0.00" type="number" className="w-full p-4 rounded-xl border bg-white font-bold" value={editProduct.price} onChange={e => setEditProduct({...editProduct, price: parseFloat(e.target.value)})} />
               </div>
               <div className="space-y-1.5 col-span-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-2">Primary Thumbnail URL</label>
                  <input placeholder="https://..." className="w-full p-4 rounded-xl border bg-white" value={editProduct.image || ''} onChange={e => setEditProduct({...editProduct, image: e.target.value})} />
               </div>
               <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-2">Category</label>
                  <select className="w-full p-4 rounded-xl border bg-white font-bold" value={editProduct.category} onChange={e => setEditProduct({...editProduct, category: e.target.value as Section})}>
               ...
               <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-4 p-4 rounded-xl border bg-white">
                    ...
                    <span className="font-bold text-zinc-600">Mark as Premium Asset</span>
                  </div>
               </div>
            </div>
            <div className="space-y-4 pt-4 border-t border-zinc-100">
               ...
               <div className="flex gap-2">
                  <input placeholder="Enter Image URL..." className="flex-1 p-4 rounded-xl border bg-white" value={galleryUrlInput} onChange={e => setGalleryUrlInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddToGallery()} />
                  <button onClick={handleAddToGallery} className="px-6 bg-zinc-900 text-white rounded-xl font-black">Add</button>
               </div>
          ...
          <div className="pt-6">
            <button onClick={handleSaveProduct} className="w-full py-5 bg-[#007AFF] text-white rounded-2xl font-black text-lg shadow-xl shadow-blue-500/20 active:scale-95 transition-all">Commit Changes to Store</button>
          </div>
        </div>
      )}
        ...
        {products.map(p => (
          <div key={p.id} className="glass-panel p-4 rounded-2xl flex items-center justify-between border border-white">
            <div className="flex items-center gap-4">
              <img src={p.image} className="w-16 h-10 rounded-lg object-cover shadow-sm" alt="" />
              <div>
                <h4 className="font-bold">{p.title}</h4>
                <div className="flex items-center gap-2">
                  <p className="text-[10px] text-zinc-400 font-black uppercase tracking-tighter">{p.category}</p>
                  <span className="text-[10px] text-blue-500 font-bold">• {(p.gallery || []).length} photos</span>
                </div>
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={() => { setEditProduct(p); setIsEditing(true); setIsEditingBanner(false); setIsChangingPassword(false); }} className="p-3 bg-blue-50 text-[#007AFF] rounded-xl"><i className="fa-solid fa-pen"></i></button>
              <button onClick={async () => { if(!supabase) return; await supabase.from('products').delete().eq('id', p.id); setProducts(pr => pr.filter(x => x.id !== p.id)); }} className="p-3 bg-red-50 text-red-500 rounded-xl"><i className="fa-solid fa-trash"></i></button>
            </div>
          </div>
        ))}
      </div>
  ...
  const renderPreviewPage = () => {
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-12 pb-32 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <header className="flex items-center justify-between">
           <button 
             onClick={() => { window.location.hash = '#/'; setActiveSection('Home'); }} 
             className="w-12 h-12 glass-panel rounded-full flex items-center justify-center text-zinc-900 border-white hover:bg-white shadow-xl transition-all"
           >
              <i className="fa-solid fa-chevron-left"></i>
           </button>
           <div className="text-center flex flex-col items-center">
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#007AFF] mb-1">{p.category} Preview</span>
              <h2 className="text-3xl font-black tracking-tighter text-zinc-900 leading-none">{p.title}</h2>
           </div>
           ...
           <div className="lg:col-span-7 space-y-6">
              <div className="relative group">
                 <div 
                    ...
                    className="flex overflow-x-auto snap-x snap-mandatory scroll-smooth no-scrollbar rounded-[3rem] shadow-2xl bg-zinc-100 border-4 border-white"
                    style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
                 >
                    {fullGallery.map((url, idx) => (
                       <div key={idx} className="min-w-full snap-center flex items-center justify-center p-4">
                          <img 
                            src={url} 
                            className="w-full h-auto rounded-[2rem] object-contain shadow-sm" 
                            alt={`Preview ${idx + 1}`} 
                          />
                       </div>
                    ))}
                 ...
                 <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex gap-2.5 px-5 py-3 bg-black/20 backdrop-blur-xl rounded-full border border-white/20">
                    {fullGallery.map((_, idx) => (
                       <div 
                          key={idx} 
                          className={`h-2 rounded-full transition-all duration-300 ${currentSlide === idx ? 'w-8 bg-[#007AFF]' : 'w-2 bg-white/50'}`}
                       />
                    ))}
                 </div>
                 ...
                 <div className="absolute top-1/2 -translate-y-1/2 left-6 opacity-0 group-hover:opacity-100 transition-opacity hidden sm:block">
                    <button 
                       ...
                       className="w-12 h-12 bg-white/80 backdrop-blur-lg rounded-full shadow-lg flex items-center justify-center text-[#007AFF]"
                    >
                       <i className="fa-solid fa-chevron-left"></i>
                    </button>
                 </div>
              ...
              <div className="flex items-center justify-center gap-4 py-2 opacity-60">
                 <i className="fa-solid fa-hand-pointer animate-bounce text-[#007AFF]"></i>
                 <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Swipe to explore all {fullGallery.length} screens</p>
              </div>
           </div>

           {/* Info & Purchase Sidebar */}
           <div className="lg:col-span-5 lg:sticky lg:top-32 space-y-8">
              <div className="glass-panel p-10 rounded-[2.5rem] border-white shadow-2xl space-y-8">
                 <div className="flex justify-between items-start">
                    <div>
                       <p className="text-[10px] font-black uppercase text-zinc-400 tracking-[0.2em] mb-1">Market Price</p>
                 ...
                 <div className="space-y-4 pt-4 border-t border-zinc-100/50">
                    <h4 className="font-black text-lg tracking-tight">Product Description</h4>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-100 flex flex-col items-center gap-2">
                       <i className="fa-solid fa-mobile-screen text-[#007AFF] text-xl"></i>
                       <span className="text-[10px] font-black uppercase text-zinc-400">Target UI</span>
                       <span className="text-xs font-black text-zinc-900">{p.compatibility || 'ColorOS / Realme UI'}</span>
                    </div>
                    <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-100 flex flex-col items-center gap-2">
                       <i className="fa-solid fa-shield-check text-[#007AFF] text-xl"></i>
                       <span className="text-[10px] font-black uppercase text-zinc-400">Security</span>
                       <span className="text-xs font-black text-zinc-900">Verified Asset</span>
                    </div>
                 </div>
      ...
      <div className="max-w-6xl mx-auto space-y-12 pb-32 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <header className="flex flex-col items-center text-center gap-8">
           <div className="max-w-2xl">
              ...
              <p className="text-zinc-500 font-medium text-lg">Complete your selections to get personalized content for your device.</p>
           </div>
             ...
             <div className="space-y-8">
                <div className="flex gap-6 group">
                   ...
                   <div className="flex-1 glass-panel p-6 sm:p-8 rounded-[2rem] border border-white shadow-xl hover:shadow-2xl transition-all">
                      <h3 className="font-black text-xl text-zinc-900 mb-6 flex items-center gap-3"><i className="fa-solid fa-microchip text-[#007AFF]"></i>Brand Selection</h3>
                      <div className="grid grid-cols-2 gap-4">
                         {['Realme', 'Oppo'].map((phone) => (
                           <button key={phone} onClick={() => setSelectedPhone(phone as any)} className={`relative py-6 rounded-2xl font-black text-xl transition-all border-2 flex flex-col items-center gap-2 overflow-hidden ${selectedPhone === phone ? 'bg-[#007AFF] text-white border-[#007AFF] shadow-xl shadow-blue-500/20' : 'bg-zinc-50/50 text-zinc-400 border-transparent hover:bg-white hover:border-zinc-200'}`}>
                              <i className={`fa-solid ${phone === 'Realme' ? 'fa-mobile-screen-button' : 'fa-mobile'} text-3xl opacity-30`}></i>{phone}
                              {selectedPhone === phone && (<div className="absolute top-2 right-2 animate-in zoom-in"><i className="fa-solid fa-circle-check text-white"></i></div>)}
                           </button>
                         ))}
                      </div>
                   </div>
                ...
                <div className="flex gap-6 group">
                   ...
                   <div className="flex-1 glass-panel p-6 sm:p-8 rounded-[2rem] border border-white shadow-xl hover:shadow-2xl transition-all">
                      <h3 className="font-black text-xl text-zinc-900 mb-6 flex items-center gap-3"><i className="fa-solid fa-layer-group text-[#007AFF]"></i>Asset Category</h3>
                      <div className="p-1.5 bg-zinc-100/80 rounded-2xl flex items-center">
                         {['Themes', 'Widgets', 'Walls'].map(cat => (
                           <button key={cat} onClick={() => setSelectedCategory(cat as Section)} className={`flex-1 py-3 rounded-xl text-sm font-black transition-all ${selectedCategory === cat ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-800'}`}>{cat}</button>
                         ))}
                      </div>
                   </div>
                </div>
                <div className="flex gap-6 group">
                   ...
                   <div className="flex-1 glass-panel p-6 sm:p-8 rounded-[2rem] border border-white shadow-xl hover:shadow-2xl transition-all">
                      <h3 className="font-black text-xl text-zinc-900 mb-6 flex items-center gap-3"><i className="fa-solid fa-cart-flatbed-suitcase text-[#007AFF]"></i>Specific Item</h3>
                      <div className="relative">
                         <select value={selectedProductId} onChange={(e) => setSelectedProductId(e.target.value)} className="w-full p-6 rounded-2xl bg-zinc-50/50 border-2 border-transparent text-zinc-900 font-black text-lg appearance-none outline-none focus:border-[#007AFF] focus:bg-white transition-all pl-8 shadow-inner">
                         ...
                         <div className="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-[#007AFF]"><i className="fa-solid fa-angles-up-down text-lg"></i></div>
                      </div>
                   </div>
                </div>
             </div>
                      ...
                      <>
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-6">
                           <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="text-white/70 text-[10px] font-bold">{selectedPhone === 'Realme' ? 'Realme UI 5.0 / 6.0' : selectedProduct.compatibility}</span>
                           </div>
                           <h3 className="text-white font-black text-2xl tracking-tight leading-none">{selectedProduct.title}</h3>
                        </div>
                      </>
                    ) : (
                      <div className="w-full h-full flex flex-col items-center justify-center text-zinc-300 gap-4"><i className="fa-solid fa-shopping-cart text-5xl opacity-20"></i><p className="font-bold text-xs">Waiting for selection...</p></div>
                    )}
                  </div>
                </div>
                {selectedProduct && (
                    ...
                    <div className="flex flex-col gap-4">
                       <div className="flex items-center justify-between p-6 bg-zinc-50/50 rounded-3xl border border-zinc-100/50">
                          <div className="text-left">
                             ...
                             <p className="text-4xl font-black text-zinc-900 tracking-tighter">{selectedProduct.price === 0 ? 'FREE' : `$${selectedProduct.price.toFixed(2)}`}</p>
                          </div>
                          <div className="text-right">
                             <div className="flex items-center gap-2 text-blue-600 font-black text-[10px] uppercase bg-blue-50 px-3 py-1.5 rounded-full mb-1"><i className="fa-solid fa-shield-check"></i><span>Verified</span></div>
                             <p className="text-zinc-400 text-[9px] font-bold">Secure Delivery</p>
                          </div>
                       </div>
                       {selectedProduct.price > 0 && (
                         <div className="p-5 bg-amber-50 rounded-2xl border border-amber-200/50 flex gap-4 animate-in fade-in zoom-in-95">
                            <div className="shrink-0 w-10 h-10 bg-amber-500 text-white rounded-xl flex items-center justify-center shadow-lg shadow-amber-500/20"><i className="fa-solid fa-wallet text-sm"></i></div>
                            <div className="space-y-1">
                               <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest leading-none">Important Instruction</p>
                               <p className="text-[11px] font-bold text-amber-800 leading-relaxed">Before sending the order message, ensure you have transferred the amount via <span className="underline decoration-amber-400 underline-offset-4 font-black">Vodafone Cash</span> to this number: <br/><span className="text-lg font-black tracking-tight text-amber-900">01091931466</span></p>
                            </div>
                         </div>
                       )}
                    </div>
  ...
  if (isLoading) return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-[#F2F2F7] gap-4">
      ...
      <div className="font-black text-2xl text-[#007AFF] animate-pulse">EDGE STORE</div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#F2F2F7]">
      ...
      {isAuthModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/20 backdrop-blur-md animate-in fade-in duration-300">
           <div className="w-full max-sm glass-panel p-8 rounded-[2.5rem] border border-white shadow-2xl space-y-6 animate-in zoom-in-95 duration-300">
              <div className="text-center space-y-2">
                 <div className="w-16 h-16 bg-zinc-900 text-white rounded-2xl flex items-center justify-center mx-auto text-2xl shadow-xl"><i className="fa-solid fa-lock"></i></div>
                 <h3 className="text-2xl font-black tracking-tight pt-2">Admin Access</h3>
                 <p className="text-zinc-400 font-bold text-sm">Please enter your password to proceed.</p>
              </div>
              <input type="password" autoFocus value={passwordInput} onChange={e => setPasswordInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAdminAuth()} placeholder="••••" className="w-full p-5 rounded-2xl bg-zinc-100 text-center text-3xl font-black tracking-[0.5em] outline-none focus:ring-4 focus:ring-blue-500/10 transition-all" />
              <div className="grid grid-cols-2 gap-3">
                 <button onClick={() => { setIsAuthModalOpen(false); setPasswordInput(''); }} className="py-4 rounded-xl font-black text-zinc-400 hover:text-zinc-600">Cancel</button>
                 <button onClick={handleAdminAuth} className="py-4 rounded-xl bg-[#007AFF] text-white font-black shadow-lg">Login</button>
              </div>
           </div>
        </div>
      )}
      <main className="max-w-7xl mx-auto px-8 py-12">
          ...
          <div className="space-y-16 pb-32">
            {banner.isVisible && (
              <section className="relative w-full aspect-video rounded-[2rem] overflow-hidden shadow-2xl group animate-in zoom-in-95 duration-700">
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/40 to-transparent flex flex-col justify-center px-12 sm:px-20">
                  <div className="max-w-2xl space-y-6">
                    <h2 className="text-5xl sm:text-7xl font-black text-white leading-tight">{banner.title} <br/><span className="text-[#007AFF]">{banner.highlight}</span></h2>
                    <button onClick={() => setActiveSection('Themes')} className="px-10 py-5 bg-[#007AFF] text-white rounded-2xl font-black text-xl shadow-xl shadow-blue-500/30">Explore Shop</button>
                  </div>
                </div>
              </section>
            )}
            <section className="space-y-10">
              <h2 className="text-3xl font-black tracking-tighter flex items-center gap-3"><div className="w-2 h-8 bg-[#007AFF] rounded-full"></div>New Arrivals</h2>
        ...
        {['Themes', 'Widgets', 'Walls'].includes(activeSection) && (
          <div className="space-y-12 pb-32 animate-in fade-in slide-in-from-bottom-4 duration-500">
             <div className="flex items-center gap-4">
                <div className="w-2 h-12 bg-[#007AFF] rounded-full"></div>
      ...
      {notification && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[200] animate-in slide-in-from-top-4">
           <div className={`px-6 py-3 rounded-full shadow-2xl font-bold flex items-center gap-3 border ${notification.type === 'success' ? 'bg-green-500 text-white border-green-600' : 'bg-[#007AFF] text-white border-blue-600'}`}>
