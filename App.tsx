...
const App: React.FC = () => {
  ...
  const saveVideo = async () => {
    ...
    try {
      ...
      showNotify("Cloud Protection Error", "error"); 
    } finally { 
  ...
  const saveProduct = async () => {
    ...
    try {
      ...
      showNotify("Marketplace Updated");
    } catch (err) { 
      showNotify("Database Access Error", "error"); 
    } finally { 
      setIsPublishing(false); 
    }
  };
  ...
  const updateSetting = async (key: string, value: string) => {
    try {
      ...
      await refreshData();
      showNotify("Security Settings Updated");
    } catch (err) { 
  ...
  const orderedProduct = useMemo(() => products.find(p => p.id === orderProductId), [products, orderProductId]);
  ...
  return (
    <div className="min-h-screen pb-32">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="space-y-20 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <section className="space-y-8">
              <div className="flex justify-between items-end px-1">
                ...
                <span className="text-[9px] font-black text-zinc-400 tracking-widest uppercase">{products.length} Assets Online</span>
              </div>
              {products.length > 0 ? (
            ...
            <section className="space-y-8">
              ...
              {videos.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {videos.map(v => (
                    <a key={v.id} href={v.url} target="_blank" rel="noopener noreferrer" className="glass-panel group rounded-[2rem] overflow-hidden hover:scale-[1.03] transition-all border-2 border-transparent hover:border-red-500/30">
                      <div className="aspect-video relative bg-zinc-800">
                        <img 
                          src={`https://img.youtube.com/vi/${v.id}/maxresdefault.jpg`} 
                          className="w-full h-full object-cover transition-transform group-hover:scale-110" 
                          onError={(e) => (e.currentTarget.src = `https://img.youtube.com/vi/${v.id}/mqdefault.jpg`)}
                        />
                      ...
                      <div className="p-5 font-black text-sm line-clamp-2 leading-tight group-hover:text-red-600 transition-colors">{v.title}</div>
                    </a>
                  ))}
                </div>
              ) : (
        ...
        {activeSection === 'Order' && (
          <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-500">
            <div className="glass-panel p-8 sm:p-12 rounded-[3rem] space-y-10 relative overflow-hidden">
               <div className="absolute top-0 right-0 w-32 h-32 bg-[#007AFF]/10 blur-3xl -z-10"></div>
               <div className="text-center space-y-2">
                 <h2 className="text-4xl font-black tracking-tight uppercase">Checkout</h2>
                 <p className="text-zinc-400 font-black text-[10px] tracking-[0.3em] uppercase">Confirm your order</p>
               </div>
               ...
               {orderedProduct ? (
                 <div className="space-y-8">
                   <div className="flex items-center gap-6 p-6 bg-zinc-100 dark:bg-zinc-800/50 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-inner">
                      <div className="w-24 h-24 rounded-2xl overflow-hidden shadow-2xl border-2 border-white dark:border-zinc-700 shrink-0">
                        <img src={orderedProduct.image} className="w-full h-full object-cover" />
                      </div>
                      <div className="flex-1">
                        <h3 className="font-black text-xl tracking-tight leading-tight">{orderedProduct.title}</h3>
                        <p className="text-[#007AFF] font-black text-xl mt-1">{orderedProduct.price === 0 ? 'FREE' : `${orderedProduct.price.toFixed(2)} EGP`}</p>
                        <div className="mt-2 text-[9px] font-black uppercase tracking-widest text-zinc-400">{orderedProduct.category}</div>
                      </div>
                   </div>
                   ...
                   <div className="space-y-4 px-2">
                      ...
                      <div className="flex justify-between font-black text-xs text-zinc-400 uppercase tracking-widest"><span>Setup Fee</span><span className="text-zinc-900 dark:text-zinc-100">0.00 EGP</span></div>
                      <div className="h-px bg-zinc-200 dark:bg-zinc-800 w-full my-4"></div>
                      <div className="flex justify-between font-black text-3xl uppercase tracking-tighter items-center">
                        <span>Total</span>
                   ...
                   <div className="grid grid-cols-1 gap-4">
                     <a href={`https://wa.me/201026419747?text=I'd like to purchase: ${orderedProduct.title}`} target="_blank" className="w-full py-6 bg-[#25D366] text-white rounded-[2rem] font-black text-lg shadow-xl shadow-green-500/20 text-center flex items-center justify-center gap-3 transition-transform active:scale-95">
                       <i className="fa-brands fa-whatsapp text-2xl"></i> WhatsApp Order
                     </a>
                     <a href="https://t.me/Mohamed_Edge" target="_blank" className="w-full py-6 bg-[#0088CC] text-white rounded-[2rem] font-black text-lg shadow-xl shadow-blue-500/20 text-center flex items-center justify-center gap-3 transition-transform active:scale-95">
                 ...
                 <div className="text-center p-20 space-y-6">
                    <i className="fa-solid fa-cart-shopping text-6xl text-zinc-200 dark:text-zinc-800"></i>
                    <p className="text-zinc-400 font-black uppercase text-xs tracking-widest">Your cart is empty</p>
                    <button onClick={() => window.location.hash = '#/'} className="px-8 py-4 bg-[#007AFF] text-white rounded-full font-black uppercase text-[10px]">Start Shopping</button>
                 </div>
               )}
            </div>
          </div>
               ...
               <div className="lg:col-span-7 space-y-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {selectedProduct.gallery.map((img, idx) => (
                      <div key={idx} className="aspect-[9/16] rounded-3xl overflow-hidden shadow-lg border-2 border-white dark:border-zinc-800 hover:scale-105 transition-transform duration-500">
               ...
               <div className="lg:col-span-5">
                 <div className="glass-panel p-8 sm:p-12 rounded-[2.5rem] space-y-6 sticky top-28">
                    ...
                    <div className="space-y-3 pt-6">
                      <div className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-zinc-400"><i className="fa-solid fa-circle-check text-[#007AFF]"></i> Instant Download</div>
                      <div className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-zinc-400"><i className="fa-solid fa-circle-check text-[#007AFF]"></i> Lifetime Access</div>
                    </div>
                    <button onClick={() => { setOrderProductId(selectedProduct.id); localStorage.setItem('last_ordered_id', selectedProduct.id); window.location.hash = '#/order'; }} className="w-full py-6 bg-[#007AFF] text-white rounded-[2rem] font-black text-xl shadow-2xl active:scale-95 transition-all mt-4">Order License</button>
                 </div>
               </div>
             </div>
           </div>
            ...
            {adminTab === 'Inventory' && (
              <div className="space-y-8">
                <div className="glass-panel p-6 rounded-[2.5rem] flex justify-between items-center">
                  ...
                  <button onClick={() => { setEditProduct({ title: '', price: 0, category: 'Themes', image: '', description: '', gallery: [] }); setIsEditingProduct(true); }} className="px-8 py-3.5 bg-[#007AFF] text-white rounded-xl font-black uppercase text-[9px] shadow-lg">Create New</button>
                </div>
                  ...
                  <div className="glass-panel p-8 rounded-[2.5rem] space-y-8 animate-in zoom-in border-2 border-[#007AFF]/20">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                             ...
                             <div className="aspect-video bg-zinc-100 dark:bg-zinc-800 rounded-3xl overflow-hidden relative group border-2 border-dashed border-zinc-300 dark:border-zinc-700">
                                {editProduct.image ? (
                                  ...
                                  <div className="w-full h-full flex flex-col items-center justify-center text-zinc-400 p-4 text-center">
                                    ...
                                    <span className="text-[9px] font-black uppercase">Click to upload cover</span>
                                  </div>
                                )}
                                <input type="file" accept="image/*" onChange={async e => { 
                                  // Fix: Explicitly cast to File to resolve unknown type error when calling fileToBase64
                                  if(e.target.files?.[0]) { 
                                 ...
                                 <div className="aspect-[9/16] rounded-lg border-2 border-dashed border-zinc-300 dark:border-zinc-700 flex items-center justify-center text-zinc-400 relative">
                                    ...
                                    <input type="file" multiple accept="image/*" onChange={async e => {
                                      if(e.target.files) {
                                        ...
                                        // Fix: Explicitly cast each item to File to fix 'unknown' type error in map callback
                       ...
                       <div className="space-y-5">
                          ...
                          <textarea className="w-full p-5 rounded-2xl bg-zinc-100 dark:bg-zinc-800 font-medium h-32 outline-none border-2 border-transparent focus:border-[#007AFF]" placeholder="Detailed description..." value={editProduct.description || ''} onChange={e => setEditProduct({...editProduct, description: e.target.value})} />
                          <div className="flex gap-4 pt-4">
                            ...
                            <button onClick={saveProduct} disabled={isPublishing} className="flex-[3] py-5 bg-[#007AFF] text-white font-black text-[10px] uppercase rounded-2xl shadow-xl transition-transform active:scale-95">{isPublishing ? 'Synchronizing...' : 'Push to Cloud'}</button>
                          </div>
                       </div>
                    </div>
                  </div>
                  ...
                  {products.length > 0 ? products.map(p => (
                    <div key={p.id} className="p-5 glass-panel rounded-[2rem] flex items-center justify-between group">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={async () => { if(window.confirm("Permanently delete this item?")) { await supabase.from('products').delete().eq('id', p.id); refreshData(); showNotify("Asset Removed"); } }} className="w-10 h-10 bg-red-500/10 text-red-500 rounded-full flex items-center justify-center hover:bg-red-500 hover:text-white transition-all"><i className="fa-solid fa-trash-can"></i></button>
                      </div>
                    </div>
                  )) : (
                    <div className="text-center py-20 text-zinc-400 font-black uppercase text-[10px] tracking-widest">No products in cloud database</div>
                  )}
                </div>
              </div>
            )}
              ...
              <div className="space-y-8">
                ...
                {isEditingVideo && (
                  <div className="glass-panel p-8 rounded-[2.5rem] space-y-6 animate-in zoom-in border-2 border-red-500/20">
                    ...
                    <input className="w-full p-5 rounded-2xl bg-zinc-100 dark:bg-zinc-800 font-black outline-none border-2 border-transparent focus:border-red-500" value={editVideo.url || ''} onChange={e => setEditVideo({...editVideo, url: e.target.value})} placeholder="YouTube URL" />
                    <div className="flex gap-4">
                      ...
                      <button onClick={saveVideo} className="flex-[3] py-4 bg-red-500 text-white font-black text-[10px] uppercase rounded-xl shadow-xl">{isPublishing ? 'Linking...' : 'Save Tutorial'}</button>
                    </div>
                  </div>
                )}
                <div className="grid grid-cols-1 gap-4">
                  {videos.length > 0 ? videos.map(v => (
                    <div key={v.id} className="p-4 glass-panel rounded-[2rem] flex items-center justify-between">
                      ...
                      <button onClick={async () => { if(window.confirm("Remove video?")) { await supabase.from('videos').delete().eq('id', v.id); refreshData(); showNotify("Video Deleted"); } }} className="w-10 h-10 text-red-500 hover:bg-red-500/10 rounded-full transition-colors"><i className="fa-solid fa-trash-can"></i></button>
                    </div>
                  )) : (
                    <div className="text-center py-20 text-zinc-400 font-black uppercase text-[10px] tracking-widest">Tutorial database is empty</div>
                  )}
                </div>
              </div>
            )}
            ...
            {adminTab === 'Settings' && (
              <div className="glass-panel p-10 rounded-[3rem] space-y-10">
                <div className="space-y-4">
                   <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest px-2">Master Password</label>
                ...
                <div className="space-y-4">
                   <label className="text-[10px] font-black uppercase text-zinc-400 tracking-widest px-2">Global Logo URL</label>
                   <div className="flex gap-4 items-center">
                     ...
                     <button onClick={() => updateSetting('site_logo', siteLogo)} className="p-6 bg-[#007AFF] text-white rounded-[2rem] font-black uppercase text-[10px]">Update</button>
                   </div>
                </div>
              </div>
            )}
      ...
      {/* Show BottomNav on every user-facing page except Preview */}
