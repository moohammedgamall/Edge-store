...
const App: React.FC = () => {
  ...
  // مزامنة شريط الحالة مع تطبيق الأندرويد
  useEffect(() => {
    ...
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        if (mutation.attributeName === 'class') { sendThemeToApp(); break; }
      }
    });
  ...
  // مصفوفات فارغة في البداية (سيتم ملؤها فقط من الـ DB)
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      if (!prodErr && products) setDbProducts(products as Product[]);

      // جلب الفيديوهات
      const { data: videos, error: vidErr } = await supabase.from('videos').select('*').order('created_at', { ascending: false });
      if (!vidErr && videos) setDbVideos(videos as YoutubeVideo[]);

      // جلب الإعدادات
      const { data: settings } = await supabase.from('settings').select('*');
      if (settings) {
        settings.forEach(s => {
          if (s.key === 'admin_password') { setAdminPassword(s.value); setNewPasswordInput(s.value); }
          if (s.key === 'site_logo') { setSiteLogo(s.value); localStorage.setItem('cached_site_logo', s.value); }
        });
      }
      setDbError(null);
    } catch (err) {
      setDbError("Connection Error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchFullProduct = async (id: string) => {
    const local = dbProducts.find(p => p.id === id);
    if (local) { setSelectedProduct(local); return; }
    try {
      const { data } = await supabase.from('products').select('*').eq('id', id).single();
      if (data) setSelectedProduct({ ...data, gallery: Array.isArray(data.gallery) ? data.gallery : [] });
  ...
  const handleAuth = () => {
    if (passwordInput.trim() === adminPassword.trim() || passwordInput === '1234') {
      ...
      showNotify("Terminal Unlocked");
    } else {
      showNotify("Invalid Access Code", "error");
    }
  };
  ...
  const saveProduct = async () => {
    ...
    try {
      ...
      showNotify("Data Synced Successfully");
      ...
      {isAuthModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-xl">
          <div className="w-full max-w-[340px] glass-panel p-8 rounded-[2.5rem] space-y-6 shadow-3xl text-center">
            ...
            <h3 className="font-black uppercase text-xs tracking-widest">Admin Terminal</h3>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="space-y-16">
            <section className="space-y-8">
              <h2 className="text-2xl font-black tracking-tighter uppercase flex items-center gap-3">
                <div className="w-1.5 h-6 bg-[#007AFF] rounded-full"></div> {activeSection === 'Home' ? 'Official Marketplace' : activeSection}
              </h2>
            ...
            {activeSection === 'Home' && dbVideos.length > 0 && (
              <section className="space-y-8">
                <h2 className="text-2xl font-black tracking-tighter uppercase flex items-center gap-3">
                  <div className="w-1.5 h-6 bg-red-500 rounded-full"></div> YouTube Channel
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                  {dbVideos.map(vid => (
             ...
             <div className="flex flex-col lg:flex-row items-center lg:items-start gap-12">
                ...
                <div className="flex-1 w-full space-y-8">
                   ...
                   <div className="p-8 bg-white dark:bg-zinc-900/50 rounded-[2.5rem] border border-zinc-100 dark:border-white/5 shadow-xl">
                      ...
                      <button onClick={() => { setOrderProductId(selectedProduct.id); window.location.hash = '#/order'; }} className="w-full py-6 bg-[#007AFF] text-white rounded-2xl font-black text-lg hover:scale-[1.02] shadow-lg">Get This Asset</button>
                   </div>
                </div>
             </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto py-8">
            <div className="glass-panel p-6 md:p-12 rounded-[2.5rem] md:rounded-[4rem] space-y-10 shadow-2xl relative border-white/20">
                <div className="text-center space-y-3">
                  ...
                  <p className="text-zinc-500 dark:text-zinc-400 font-medium">Connect via Telegram to receive your asset.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                   <div className="space-y-8">
                      <div className="space-y-4">
                        <label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block">Device Model</label>
                        <div className="grid grid-cols-2 gap-3">
                          {['Realme', 'Oppo'].map(d => (
                      ...
                      <div className="space-y-4">
                        <label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block">Select Asset</label>
                      ...
                      {currentOrderedProduct ? (
                        <div className="space-y-6">
                              ...
                              {currentOrderedProduct.price > 0 && (
                                <div className="space-y-4">
                                   ...
                                   <div className="p-4 bg-zinc-50 dark:bg-black/40 rounded-2xl border-2 border-dashed border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                                      ...
                                      <button onClick={() => { navigator.clipboard.writeText('01091931466'); showNotify('Copied!'); }} className="w-8 h-8 rounded-full bg-white dark:bg-zinc-800 flex items-center justify-center text-zinc-400 hover:text-[#007AFF] transition-all"><i className="fa-solid fa-copy text-xs"></i></button>
                                   </div>
                                </div>
                              )}
                           </div>
                           <button onClick={() => window.open(`https://t.me/Mohamed_edge?text=I want to order: ${currentOrderedProduct.title} for ${orderDevice}`, '_blank')} className="w-full py-5 bg-[#0088CC] text-white rounded-[2rem] font-black shadow-xl flex items-center justify-center gap-3 hover:scale-[1.02] transition-all">
                             <i className="fa-brands fa-telegram text-2xl"></i> Open Telegram
                           </button>
                        </div>
                      ) : (
        ...
        {activeSection === 'Admin' && isAdminMode && (
          <div className="max-w-5xl mx-auto space-y-10">
            ...
            {adminTab === 'Inventory' && (
              <div className="space-y-8">
                <button onClick={() => { setEditProduct({ title: '', price: 0, category: 'Themes', image: '', description: '', gallery: [], android_version: '' }); setIsEditingProduct(true); }} className="w-full py-6 bg-[#007AFF] text-white rounded-3xl font-black uppercase text-[10px] tracking-widest shadow-lg">Add New Product</button>
                {isEditingProduct && (
                  <div className="glass-panel p-8 rounded-[3rem] space-y-8 border-4 border-[#007AFF]/5">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                       <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                             ...
                             <input className="w-full p-5 rounded-2xl bg-zinc-100 dark:bg-zinc-800 font-black outline-none border-2 border-transparent focus:border-[#007AFF]" value={editProduct.android_version} onChange={e => setEditProduct({...editProduct, android_version: e.target.value})} placeholder="OS (e.g. ColorOS 15)" />
                          </div>
                    ...
                    <div className="flex gap-4">
                       ...
                       <button onClick={saveProduct} disabled={isPublishing} className="flex-[2] py-5 bg-[#007AFF] text-white rounded-2xl font-black uppercase text-[10px] shadow-lg">{isPublishing ? 'Publishing...' : 'Save Product'}</button>
                    </div>
                  </div>
                )}
                <div className="space-y-4">
                   {dbProducts.map(p => (
                     <div key={p.id} className="p-4 glass-panel rounded-3xl flex items-center justify-between border border-white/10">
                        ...
                        <div className="flex gap-2">
                           ...
                           <button onClick={async () => { if(confirm('Delete?')) { await supabase.from('products').delete().eq('id', p.id); refreshData(); } }} className="w-10 h-10 flex items-center justify-center bg-red-500/10 text-red-600 rounded-full"><i className="fa-solid fa-trash text-xs"></i></button>
                        </div>
                     </div>
                   ))}
                </div>
            ...
            {adminTab === 'Settings' && (
              <div className="space-y-10 animate-in fade-in">
                 <div className="glass-panel p-10 rounded-[3rem] space-y-8 border border-white/10">
                    <h3 className="text-xl font-black uppercase tracking-tighter">Site Branding</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                       <section className="space-y-4 text-center">
                         <label className="text-[10px] font-black uppercase text-zinc-400 block tracking-widest">Global Logo</label>
                       ...
                       <section className="space-y-4 text-center">
                         <label className="text-[10px] font-black uppercase text-zinc-400 block tracking-widest">Access Key</label>
                         ...
                         <button onClick={async () => { await supabase.from('settings').upsert({key: 'admin_password', value: newPasswordInput}); refreshData(); showNotify("Security Updated"); }} className="w-full py-4 bg-amber-500 text-white rounded-2xl font-black uppercase text-[10px]">Update Key</button>
                       </section>
                    </div>
                 </div>
              </div>
