...
import { NAV_ITEMS, MOCK_PRODUCTS, MOCK_VIDEOS } from './constants';
import BottomNav from './components/BottomNav';
import Header from './components/Header';
import ProductCard from './components/ProductCard';

// ملاحظة هامة: تأكد من أن هذه القيم تتطابق مع مشروعك في Supabase
// إذا كنت متأكداً من صحتها، فالسبب الأرجح هو عدم وجود الجداول (products, videos, settings) في قاعدة البيانات.
...
const App: React.FC = () => {
  ...
  // البدء بالبيانات التجريبية كحالة افتراضية
  const [dbProducts, setDbProducts] = useState<Product[]>(MOCK_PRODUCTS);
  const [dbVideos, setDbVideos] = useState<YoutubeVideo[]>(MOCK_VIDEOS); 
  ...
  const refreshData = async () => {
    ...
    try {
      // 1. جلب المنتجات (محاولة واحدة لكل طلب لمنع الانهيار الكامل)
      ...
      if (prodErr) {
        console.warn("Table 'products' missing or error:", prodErr.message);
        hasError = true;
      ...
      if (vidErr) {
        console.warn("Table 'videos' missing or error:", vidErr.message);
      } else if (videos && videos.length > 0) {
        setDbVideos(videos as YoutubeVideo[]);
      }
      ...
      if (hasError) {
        setDbError("Database not fully initialized. Using local assets.");
      } else {
        setDbError(null);
      }

    } catch (err: any) {
      console.error("General Supabase Error:", err);
      // لا نضع خطأ يمنع التصفح، بل نكتفي بالبيانات التجريبية
      setDbError("Offline Mode: Connection could not be established.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchFullProduct = async (id: string) => {
    try {
      // محاولة البحث أولاً في البيانات المحملة محلياً أو من الـ DB
  ...
  const updateSetting = async (key: string, value: string) => {
    ...
    } catch (err: any) { showNotify("DB Update failed - Check console", "error"); }
  };
  ...
  const saveProduct = async () => {
    ...
    try {
      ...
      showNotify("DB Write failed - Ensure table 'products' exists", "error"); 
  ...
  return (
    <div className="min-h-screen pb-32">
      ...
      {/* شريط تحذيري بسيط في الأعلى إذا كان هناك مشكلة في الـ DB (يظهر للأدمن فقط أو كتحذير خفيف) */}
      {dbError && isAdminMode && (
        <div className="bg-amber-500/10 border-b border-amber-500/20 py-2 text-center">
           <p className="text-[9px] font-black uppercase tracking-widest text-amber-600 flex items-center justify-center gap-2">
             <i className="fa-solid fa-triangle-exclamation"></i>
             Database not connected. Using local storage.
           </p>
        </div>
      )}
